...
source = '/nas/volume1/2photon/RESDATA/test_motion_correction';
output_path = fullfile(source, 'NoRMCorre', 'results');
if ~exist(output_path)
    mkdir(output_path)
end
filename = 'file_20160830JR017W_6.tif';
...
options_rigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'bin_width',50,'max_shift',30,'us_fac',50,...
                                    'correct_bidir', true, 'max_dev', 5);
...
options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'grid_size',[32,32],'mot_uf',4,'bin_width',50,'max_shift',30,'max_dev',5,'us_fac',50);
